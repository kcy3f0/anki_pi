...
# 嘗試解析 Commit Hash
# 格式範例: backups/flashcards_20260522_150953_d6a29c9_update.db
# 我們取出檔名中含有 commit hash 的部分 (倒數第二個底線分隔的部分)
BASENAME=$(basename "$SELECTED_FILE")
# 取得以底線分隔的倒數第二個部分 (如 d6a29c9)
COMMIT_HASH=$(echo "$BASENAME" | awk -F'_' '{print $(NF-1)}')
...
echo -e "\n${GREEN}=== 還原程序結束 ===${NC}"
echo "請記得重新啟動服務以確保變更生效 (sudo systemctl restart anki_pi)"

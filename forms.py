...
class FolderForm(FlaskForm):
    name = StringField('資料夾名稱', validators=[DataRequired(message="請輸入資料夾名稱")])
...
class CardForm(FlaskForm):
    front = StringField('正面 (英文)', validators=[DataRequired(message="請輸入正面英文")])
    back = TextAreaField('背面 (中文)', validators=[DataRequired(message="請輸入背面中文")])
    card_type = SelectField('卡片類型', choices=[('recognize', '只要認得 (recognize)'), ('spell', '需要會拼 (spell)')], validators=[DataRequired()])
    decks = MultiCheckboxField('所屬牌組', coerce=int, validators=[DataRequired(message="請至少選擇一個牌組")])
